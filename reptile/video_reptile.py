...
def construct_api(user_id, _rticket, ts, sec_user_id):
    ...
    :return: api
    """
    video_api = "https://aweme.snssdk.com/aweme/v1/aweme/post/?" \
               "source=0&user_avatar_shrink=96_96&video_cover_shrink=248_330&" \
               "publish_video_strategy_type=2&max_cursor=0&" \
               "sec_user_id={}&show_live_replay_strategy=1&" \
               "count=20&is_order_flow=0&page_from=2&longitude=106.56358166666665&latitude=26.001249999999995&" \
               "location_permission=true&version_name=15.2.0&ts={}&" \
               "device_type=SM-N935F&iid=1355042122629533&app_type=normal&resolution=1280*720&" \
               "aid=1128&app_name=aweme&appTheme=dark&_rticket={}&device_platform=android&" \
               "version_code=150200&dpi=240&openudid=d4258bf9c3605799&minor_status=0&" \
               "cdid=051cbf12-37d1-4a38-aa3e-b7de8c6abf92&cpu_support64=false&ssmix=a&os_api=19&" \
               "mcc_mnc=46007&device_id=58916287376&device_brand=samsung&manifest_version_code=150201&" \
               "os_version=4.4.2&host_abi=armeabi-v7a&update_version_code=15209900&ac=wifi&language=zh&" \
               "uuid=359090010212254&channel=aweGW".format(sec_user_id, ts, _rticket)
    return video_api
...
def video_main(sec_user_id):
    ...
    try:
        ...
        print(js)
        for data in js["aweme_list"]:
            timeData = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(data["create_time"]))
            returnData.append(timeData)
...
if __name__ == '__main__':
    res = get_user_detail_info(settings.cookie, settings.query, settings.token, settings.user_agent, settings.user_id,
                               "MS4wLjABAAAAUjkYlN_ZMTdaK2XF8_haXcxeqQQbyj85OMkeWCZ97bY", settings.gorgon)
    print("--------json数据--------")
    js = json.loads(res.encode('utf-8', 'replace'))
    print(js)
    ...
    print("--------Time--------")
    for data in js["aweme_list"]:
        print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(data["create_time"])))
    print("==============用户分隔符==============")
